...
document.getElementById('director-nombre').addEventListener('input', soloLetras);


function goStep2() {
  const nombre   = document.getElementById('director-nombre').value.trim();
  const correo   = document.getElementById('director-correo').value.trim();
  const facultad = document.getElementById('director-facultad').value;

  if (!nombre)   return showErr('err-step1', 'Por favor ingresa nombre del director.');
  ...
  `Facultad: ${facultad}<br>Director: ${normalizar(nombre)}`;
...
function renderResumen(docentes) {
  const facultad = document.getElementById('director-facultad').value;
  const director   = normalizar(document.getElementById('director-nombre').value);

  document.getElementById('resumen-header').innerHTML = `
    <div class="resumen-dato"><strong>Facultad:</strong> <span>${facultad}</span></div>
    <div class="resumen-dato"><strong>Director:</strong> <span>${director}</span></div>
...
async function submitTodo() {
  ...
  const facultad   = document.getElementById('director-facultad').value;
  const director     = normalizar(document.getElementById('director-nombre').value);
  const directorCorr = document.getElementById('director-correo').value.trim().toLowerCase();
