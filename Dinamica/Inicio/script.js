...
function seleccionarPreguntasParaPartida(todasLasPreguntas, cantidadSolicitada) {
  ...
  ['FACIL', 'MEDIA', 'DIFICIL'].forEach((nivel) => {
    const necesarias = distribucionDificultad[nivel];

    for (let i = 0; i < necesarias; i++) {
      // En cada pregunta preferimos el área que esté más lejos de
      // cumplir su cuota, para que el balance M/C quede parejo al final.
      ...
      const pregunta = pools[nivel][areaPreferida].pop() || pools[nivel][areaAlterna].pop();
...
function manejarTiempoAgotado() {
  ...
  botones.forEach((boton) => {
    ...
    boton.dataset.estado = esLaCorrecta ? 'correcta-tiempo' : 'apagada';
  });
